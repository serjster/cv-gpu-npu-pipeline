...
- [x] `src/lowlatcv/pipeline/tracker.py` — `Tracker` Protocol
- [x] Extend `Track` with `state: TrackState`, `age: int`, `last_seen_frame: int`, `history: tuple[BBox, ...]`
- [x] `TrackState` enum: `TENTATIVE`, `ACTIVE`, `LOST`, `DEAD`
- [x] `ByteTracker` backend — IoU + low-score recovery; Kalman motion model optional
- [x] `Tracker.from_config` Factory Method
- [x] Wire `Detector → Tracker → Overlay`; overlay now draws `track_id` per box
- [x] Test: ID persists across consecutive frames with overlapping detections (deterministic synthetic input)
- [x] Test: state transitions `TENTATIVE → ACTIVE` after `min_hits` frames
- [x] Test: `ACTIVE → LOST` when no match for `max_age` frames
- [x] Test: `LOST → DEAD` after configurable timeout, no longer rendered
- [x] Test: ID does not get reused when a track moves to `DEAD`
